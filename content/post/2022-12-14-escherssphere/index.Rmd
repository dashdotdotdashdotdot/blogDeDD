...
Escher rotated the sphere forward by 45 degrees, and he used orthographic projection. In order for the ribbons to be never ending one will need a function that maps "latitude" to "longitude" such that a latitude of +/- 90 degrees goes to a longitude of +/- infinity.  See note on the side for how we are using these terms and the function that we use.^[We are using _latitude_ (lat) and _longitude_ (long) in more or less the same way they are used in geography.  A negative _longitude_ goes to the west, and a _longitude_ of $X + \lambda 360$ is the same as $X$ whenever $\lambda$ is an integer. The formula that we use to create the ribons is:
