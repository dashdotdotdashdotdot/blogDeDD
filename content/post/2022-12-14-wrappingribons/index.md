...
well but this we will view as fixed over time.  The basic insight is that the pattern one sees is determined by the ratio of how long it takes the turntable to do a complete revolution to how long it takes the clock to do a complete revolution. If this ratio is irrational, then the curve will never reconnect with itself.
...
`\(f_1\)` places the position of the hand of the clock on the clock using two dimensional polar coordinates in the `\(XY\)` plan.
The `\(f_1\)`: `\(R \rightarrow R^3\)` is defined as
$$ f_1( \theta_1) = [r_1sin(\theta_1), r_1cos(\theta_1), 0] $$
`\(f_2\)` rotates the plane of the clock using a rotation matrix:
...
`\(f_3\)` is determining the position of the center of the clock in the XZ plane using two dimensional polar coordinates.
...
2)  Conversely, if p/q is not a rational number they will never coincide
    and the curve will go one forever. (If it did coincide at some
...
5)  Suppose `\(p/q = 3/5\)`. The when `\(pq\)` = 15 they coincide. So that when
    ...
    they will coincide. They will also coincide when the clock goes
